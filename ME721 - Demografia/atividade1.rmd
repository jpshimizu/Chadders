...
#Exercício 2
```{r}
df2 <- df_idade_juntas %>% mutate(rate = as.integer(Masculino)/as.integer(Feminino) - 1)
df2 %>% ggplot(aes(x = Faixa.Etária.detalhada, y = rate)) +
  # agregar el argumento color = "white", si gustan
    geom_col(position = "stack", alpha = 0.6) + 
    coord_flip() +
  # colores que me gustan
  ...
  # etiquetas en positivo
    ...
    labs(
      y = "Razão",
      x = "Faixa Etária",
      title = "Razão Masculino/Feminino Ceará 1980")
```
