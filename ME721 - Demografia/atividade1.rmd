...
```{r, Leitura e definição dos dados}
path_df_idade = 'A171441189_28_143_208.csv'
df_idade = read.csv(path_df_idade,sep=';', skip=3, nrows=33, header=T)
colnames(df_idade)[1] <- 'Faixa.Etária.detalhada'
...
# Exercício 2
```{r}
df2 <- df_idade %>% mutate(rate = -as.integer(Masculino)/as.integer(Feminino))
df102 <- df_idade10 %>% mutate(rate = as.integer(Masculino)/as.integer(Feminino))

r <- as.data.frame(rbind(df2,df102)); r
r$ano <-as.factor(c(rep(1980, 33), rep(2010,33)))
r$Faixa.Etária.detalhada <- ordered(r$Faixa.Etária.detalhada, levels = c(r$Faixa.Etária.detalhada[1:33]))

r %>% ggplot(aes(x = Faixa.Etária.detalhada, y = rate, fill = ano)) + 
  geom_bar(data = subset(r, ano == "1980"), stat = "identity",  alpha = .6) + 
  geom_bar(data = subset(r, ano == "2010"), stat = "identity",  alpha = .6) +  
  scale_fill_manual(values = c("midnightblue", "darkred")) +
  theme_minimal() +
  labs(y = "Razão", x = "Faixa Etária", title = "Razão Masculino/Feminino Ceará 1980/2010") +
